...
public sealed class CachedPermissionResolver : IPermissionResolver, IDisposable
{
    ...
    private readonly ConcurrentDictionary<string, HashSet<string>> _compositeKeyIndex
    ...
    public void InvalidateRoleGroup(string roleGroupCode)
    {
        ...
        List<string> affectedCompositeKeys;
        lock (_indexLock)
        {
            if (!_compositeKeyIndex.TryRemove(roleGroupCode, out var compositeKeys))
                return;
            affectedCompositeKeys = compositeKeys.ToList();

            foreach (var compositeKey in compositeKeys)
            {
                var codes = ExtractCodes(compositeKey);
                foreach (var code in codes)
                {
                    if (string.Equals(code, roleGroupCode, StringComparison.OrdinalIgnoreCase))
                        continue;
                    if (_compositeKeyIndex.TryGetValue(code, out var otherSet))
                        otherSet.Remove(compositeKey);
                }
            }
        }

        foreach (var key in affectedCompositeKeys)
            _cache.Remove(key);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            TrackCompositeKey(cacheKey, roleGroupCodes);
            return permissions;
        }
        catch (Exception ex)
        {
    ...
    private void TrackCompositeKey(string compositeKey, List<string> roleGroupCodes)
    {
        lock (_indexLock)
        {
            foreach (var code in roleGroupCodes)
            {
                if (!_compositeKeyIndex.TryGetValue(code, out var set))
                {
                    set = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                    _compositeKeyIndex[code] = set;
                }
    ...
    private static IEnumerable<string> ExtractCodes(string compositeKey)
    {
        var prefix = "perm:";
        if (!compositeKey.StartsWith(prefix, StringComparison.OrdinalIgnoreCase))
            return [];
        return compositeKey[prefix.Length..].Split(',');
    }
