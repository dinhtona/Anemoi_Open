...
public sealed class GetLeavePoliciesHandler(ISqlRepository<LeavePolicy> repository, LeaveMapper mapper)
    : IQueryHandler<GetLeavePoliciesQuery, PaginationResponse<LeavePolicyResponse>>
{
    public async Task<PaginationResponse<LeavePolicyResponse>> Handle(GetLeavePoliciesQuery request,
        CancellationToken cancellationToken)
    {
        var page = await repository.GetManyByConditionWithPaginationAsync(
            x => (string.IsNullOrEmpty(request.SearchKey) || x.Code.Contains(request.SearchKey) ||
                    x.Name.Contains(request.SearchKey)) &&
                (string.IsNullOrEmpty(request.LeaveTypeCode) || x.LeaveTypeCode == request.LeaveTypeCode) &&
