...
public sealed class CancelPayrollRunHandler(
    ISqlRepository<PayrollRun> payrollRunRepository,
    IUnitOfWork unitOfWork,
    PayrollMapper mapper)
    : ICommandHandler<CancelPayrollRunCommand, OneOf<PayrollRunDetailResponse, ErrorDetailResponse>>
{
    public async Task<OneOf<PayrollRunDetailResponse, ErrorDetailResponse>> Handle(
        CancelPayrollRunCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (!run.Cancel(request.CancelledBy ?? PayrollConstants.SystemActor, DateTime.UtcNow, request.CancellationReason))
