...
public sealed class DeactivateLeavePolicyHandler(
    ISqlRepository<LeavePolicy> repository,
    IUnitOfWork unitOfWork)
    : ICommandHandler<DeactivateLeavePolicyCommand, OneOf<SuccessResponse, ErrorDetailResponse>>
{
    public async Task<OneOf<SuccessResponse, ErrorDetailResponse>> Handle(
        DeactivateLeavePolicyCommand request, CancellationToken cancellationToken)
    {
