...
public sealed class ArchiveEmployeeHandler(
    ...
    : ICommandHandler<ArchiveEmployeeCommand, OneOf<None, ErrorDetailResponse>>
{
    public async Task<OneOf<None, ErrorDetailResponse>> Handle(
        ArchiveEmployeeCommand request, CancellationToken ct)
    {
        ...
        try
        {
            employee.Archive(request.CreatedBy ?? PayrollConstants.SystemActor);
        }
