...
public sealed class RejectCandidateApplicationHandler(
    ISqlRepository<CandidateApplication> applicationRepository,
    IUnitOfWork unitOfWork,
    RecruitmentMapper mapper)
    : ICommandHandler<RejectCandidateApplicationCommand, OneOf<CandidateApplicationResponse, ErrorDetailResponse>>
{
    public async Task<OneOf<CandidateApplicationResponse, ErrorDetailResponse>> Handle(
        RejectCandidateApplicationCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (!app.MoveToStage(CandidateApplicationStageCode.Rejected, new CandidateApplicationStageHistoryId(IdGenerator.NextGuid()), request.ChangedBy, DateTime.UtcNow, request.Note))
