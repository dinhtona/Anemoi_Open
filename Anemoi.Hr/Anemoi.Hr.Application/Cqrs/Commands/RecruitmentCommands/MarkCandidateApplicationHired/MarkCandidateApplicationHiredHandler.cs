...
public sealed class MarkCandidateApplicationHiredHandler(
    ISqlRepository<CandidateApplication> applicationRepository,
    IUnitOfWork unitOfWork,
    RecruitmentMapper mapper)
    : ICommandHandler<MarkCandidateApplicationHiredCommand, OneOf<CandidateApplicationResponse, ErrorDetailResponse>>
{
    public async Task<OneOf<CandidateApplicationResponse, ErrorDetailResponse>> Handle(
        MarkCandidateApplicationHiredCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (!app.MoveToStage(CandidateApplicationStageCode.Hired, new CandidateApplicationStageHistoryId(IdGenerator.NextGuid()), request.ChangedBy, DateTime.UtcNow, request.Note))
