...
public sealed class BlacklistCandidateHandler(
    ISqlRepository<Candidate> candidateRepository,
    IUnitOfWork unitOfWork,
    RecruitmentMapper mapper)
    : ICommandHandler<BlacklistCandidateCommand, OneOf<CandidateResponse, ErrorDetailResponse>>
{
    public async Task<OneOf<CandidateResponse, ErrorDetailResponse>> Handle(
        BlacklistCandidateCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (!candidate.Blacklist(request.UpdatedBy, DateTime.UtcNow))
