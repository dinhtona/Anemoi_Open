...
public sealed class MoveCandidateApplicationToInterviewHandler(
    ISqlRepository<CandidateApplication> applicationRepository,
    IUnitOfWork unitOfWork,
    RecruitmentMapper mapper)
    : ICommandHandler<MoveCandidateApplicationToInterviewCommand, OneOf<CandidateApplicationResponse, ErrorDetailResponse>>
{
    public async Task<OneOf<CandidateApplicationResponse, ErrorDetailResponse>> Handle(
        MoveCandidateApplicationToInterviewCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (!app.MoveToStage(CandidateApplicationStageCode.Interview, new CandidateApplicationStageHistoryId(IdGenerator.NextGuid()), request.ChangedBy, DateTime.UtcNow, request.Note))
