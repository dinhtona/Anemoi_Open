...
- Permission display rule: Permission codes are internal and stable wire values. Permission name/description shown to users must come from API-localized permission metadata, not hardcoded in frontend translation dictionaries. Frontend must never hardcode permission name translation mappings. When API metadata is unavailable, fall back to readable derived names (e.g., "hr.leave.request.view" → "Leave Request View"), never raw `PermissionDescription*` keys.
